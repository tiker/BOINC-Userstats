...
$sum_total=mysql_result($result,0,0);
...
$result=mysql_query($query);
$sum_pendings=mysql_result($result,0,0);
...
$result_world_rank=mysql_query("SELECT rank, rank_team from boinc_werte_day WHERE project_shortname='gesamt' ORDER BY time_stamp DESC LIMIT 1");  //alle Userdaten einlesen
	while($row=mysql_fetch_assoc($result_world_rank)){
		$sum_rank_total = $row["rank"];
		$sum_rank_total_avg = $row["rank_team"];
	}

$result_grundwerte=mysql_query("SELECT * from boinc_grundwerte ORDER by project ASC");  //alle Projektgrunddaten einlesen
...
#	$gestern_ende = mktime($timezoneoffset, 0, 0, date("m"), date ("d"), date("Y"));
	...
	$result_output=mysql_query ("SELECT time_stamp, credits from boinc_werte where project_shortname='gesamt'");
		while($row=mysql_fetch_assoc($result_output)){
	...
	$result_output_gesamt=mysql_query ("SELECT time_stamp, total_credits, pending_credits, rank, rank_team from boinc_werte_day where project_shortname='gesamt'");
		while($row2=mysql_fetch_assoc($result_output_gesamt)){
		    	    ...
		    	    <h1 class="title"><font color="white"><?php echo "$tr_th_bp" ?></font></h1>
	                    <h3><font color="white"><?php echo "$project_username" . " " . $tr_th_ot . " " . $project_teamname ?></font></h3>
							...
							<?php //Link zu Boinctasks
								if ($has_boincstats == "1") {
									echo '<a href="' . $boincstats_link . '" target="_new" class="btn btn-neutral btn-simple"><i class="fa fa-bar-chart"></i> ' .$link_name_boincstats. '</a>';
								};
							?>							
                            <br />
 							<?php //Link zu Team
								if ($has_team_hp == "1") {
									echo '<a href="' . $team_hp . '" target="_new" class="btn btn-neutral btn-simple"><i class="fa fa-link"></i> ' .$link_name_teamname. '</a>';
								};
							?>	
	...
	<div class="tab-content text-center">
		<div id="projekte" class="tab-pane fade in active">
			<div class="section text-center section-default">
				...
				<table class="table table-striped table-hover text-right table-condensed" style="background: linear-gradient(to bottom, #FFFFFF 70%, #F3F3F3 100%); box-shadow: 0 1px 2px rgba(0,0,0,0.4);">
				...
				?>
					...
					<tr class="alert-info">
						...
						<td class="hidden-xs"><b><?php echo number_format($sum1h_total,0,$dec_point,$thousands_sep) ?></b></td>
						<td class="hidden-xs hidden-sm"><b><?php echo number_format($sum2h_total,0,$dec_point,$thousands_sep) ?></b></td>
						<td class="hidden-xs hidden-sm"><b><?php echo number_format($sum6h_total,0,$dec_point,$thousands_sep) ?></b></td>
						<td class="hidden-xs"><b><?php echo number_format($sum12h_total,0,$dec_point,$thousands_sep) ?></b></td>
